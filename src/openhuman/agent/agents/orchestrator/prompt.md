...
<!-- F-21 fix 2: the "MCP and Composio tool failures — surface verbatim" section
     is now sourced from `agent/prompts/structured_tool_errors.md` and appended
     in `orchestrator/prompt.rs::build` so every agent that surfaces structured
     tool errors ships the SAME rules from a single canonical fragment. See
     `agent::agents::loader::tests::structured_tool_errors_fragment_must_be_*`. -->
