...
fn re_open_is_idempotent_and_does_not_duplicate_ledger_rows() {
    ...
    with_connection(&config, |conn| {
        ...
        assert_eq!(
            count, 11,
            "eleven migrations must record exactly eleven rows (F2-14 added 004, F2-7b added 005, \
             Phase 2.5 T-1 added 006, Phase 2.5 T-4 added 007, Phase 4 F4-2 added 008, \
             F4-8 added 009, F4-9 added 010, F3-6 chunk 2 added 011)"
        );
        Ok(())
    })
    .unwrap();
