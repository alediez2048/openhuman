...
> ticket set exists so a Phase 3 kickoff doesn't start from a blank
> slate.
...
- **Watch a run light up the graph** in real time as each node
  fires.
- **Use Phase 3 node kinds**: `transform` (JSON shaping between
...
| Ticket | Title |
|---|---|
| F3-1 | Canvas scaffold — @xyflow/react integration + `/workflows/<id>/canvas` read-only render |
| F3-2 | Node palette sidebar with drag-source for every NodeKind |
| F3-3 | Per-node config drawer (click-to-edit on the canvas) |
| F3-4 | Edge wiring + DAG validation (drop the Phase 2 linear-chain assumption) |
| F3-5 | Live run highlighting via `WorkflowRunStepStarted` / `WorkflowRunStepCompleted` events |
| F3-6 | `transform` node kind (JSONPath / jq-style shaping) |
| F3-7 | `await_human_approval` node kind + approval UI surface |
| F3-8 | `fan_out` node kind + parallel-children executor |
| F3-9 | Canvas-driven create flow — "New workflow" button opens blank canvas |
| F3-10 | Phase 3 hero E2E (build a fan-out workflow via canvas) + DEVLOG closure + ADR drift audit |

---

## Pre-Phase-3 brainstorm

Decide before starting F3-1:
...
- **Event bus** publishes `WorkflowRunStepStarted` /
  `WorkflowRunStepCompleted` — Phase 3 just subscribes in the
