# F3 Initiative Primer — Browser Agent Capabilities

**For:** Future coding-agent session(s)
**Project:** OpenHuman — Workflows & Automations
**Date drafted:** 2026-05-22 (renumbered from F4 to F3 on the same date after Option-B ordering decision)
**Dependencies:** Phase 2 (multi-node workflows, F2-1 → F2-16) MUST be shipped first. Phase 4 (Canvas Editor, F4-1 → F4-10) is **NOT a prerequisite** — per the 2026-05-22 ordering decision (Option B), browser-agent is the next implementation priority after Phase 2; canvas is demand-gated and may never ship. Do not start Phase 3 work until Phase 2 is on `main`.
**Estimated total scope (Phase 3.1):** 3–6 weeks of focused work across 7 sub-tickets (`F3-1` through `F3-7`).
**Estimated total scope (Phase 3.2 follow-up, multi-runtime):** +2–3 weeks. Not covered by this primer in detail — see "Phase 3.2 — explicitly deferred" section.
...
The browser is the user's **already-authenticated CEF session**. No separate Chromium process for Phase 3.1, no cloud browser, no JS injection (per CLAUDE.md's standing constraint). The agent drives the user's real logged-in tabs.
...
By the time Phase 3 starts, the following must be on `main`:
...
- **Phase 4** — Canvas Editor (F4-1 → F4-10) is **NOT required**. Per the 2026-05-22 Option-B ordering decision, canvas is demand-gated; browser-agent ships before it (and may ship without it ever existing). Browser-action UI affordances are surfaced in the existing list view + chat preview surfaces — see F3-4 for the integration approach.

### Existing infrastructure Phase 3 can build on
...
- `app/src-tauri/src/*_scanner/` — per-provider modules already drive CDP from Rust (Telegram-web, WhatsApp-web, Slack, Discord, LinkedIn, etc.). Phase 3 generalizes the CDP wrapper.
...
- `src/openhuman/connections/types.rs::ConnectionRef::Webview` — already exists; Phase 3 adds a new variant or repurposes Webview to carry a "browser-agent-capable" flag.
- `src/openhuman/tools/impl/browser/native_backend.rs` — existing browser tooling for Composio-routed browser actions. Phase 3 generalizes / extends this.
...
## Architecture — Direction A (Phase 3.1, recommended)
...
| Fork | Options | Recommendation for Phase 3.1 |
|---|---|---|
| **Runtime location** | CEF / headless Playwright / cloud (Browserbase) | **CEF** — reuses authenticated sessions, integrates with the existing scanner infrastructure, no JS injection needed. Other runtimes are Phase 3.2. |
| **Grounding strategy** | DOM-first / vision-first / hybrid | **DOM-first with accessibility-tree augmentation; vision is opt-in fallback (F3-7) for ambiguous cases.** DOM-first is dramatically cheaper in tokens. Vision is opt-in not opt-out — guards token costs. |
...
| **Workflow integration** | New node kind + new chat tool | **Both.** `NodeKind::BrowserAction` for scheduled/unattended; `browser_act`/`browser_observe`/`browser_extract` for ad-hoc chat use (gated by feature flag for the chat surface in Phase 3.1; opens up in Phase 3.2). |
| **Safety model** | Live preview / dry-run / per-action confirmation / domain whitelist | **All four, layered.** Live preview by default; dry-run for first run of any workflow; per-action confirmation auto-enabled on financial/legal/government domains via whitelist; cost cap as hard ceiling. Detail in F3-6. |

### Why not Direction B (hybrid Playwright) for Phase 3.1?

- Phase 3.1's thesis is "act under the user's authenticated session" — the differentiating capability. Playwright as a separate runtime can't easily inherit the user's logged-in CEF session without copy-paste cookie hacks that get fragile.
...
- Generic web research (where Playwright would shine) is the **second-most-important** F4 use case, not the first. Defer to Phase 3.2.

If Phase 3.2 demand materializes (user asks for "research-mode" workflows that don't need authenticated sessions), add a `BrowserRuntime` enum + per-task runtime selection. The architecture above is forward-compatible — the CDP primitives layer is the same shape regardless of which Chrome you're driving.

### Why not Direction C (Browserbase) for Phase 3.1?
...
- If commercial cloud-browser becomes worth it (large-scale headless research), it's a Phase 3.2+ option — explicitly NOT a Phase 3.1 path.

---

## Sub-ticket breakdown (Phase 3.1)
...
|---|---|---|---|
| [`F3-1`](./F3-1.md) | CDP automation primitives (Rust core) | — | 3–5 days |
| [`F3-2`](./F3-2.md) | Page perception (DOM + a11y tree grounding) | F3-1 | 3–4 days |
| [`F3-3`](./F3-3.md) | LLM-facing tools (`browser_observe`/`act`/`extract`) | F3-1, F3-2 | 4–6 days |
...
| [`F3-6`](./F3-6.md) | Safety preamble + dry-run + cost caps + audit log | F3-3 | 3–4 days |
| [`F3-7`](./F3-7.md) | Vision-grounded fallback (Anthropic computer-use style) | F3-2, F3-3 | 4–6 days |

Total: **23–33 working days** for Phase 3.1.

### What ships at each milestone

- After **F3-3**: an OpenHuman power user can call `browser_observe` / `browser_act` / `browser_extract` from a workflow's `agent_prompt` node. No UI, no safety guards — internal-use only. Validates the core primitives work.
- After **F3-4**: workflows can declare a first-class `BrowserAction` node alongside `agent_prompt`. Multi-node workflows mixing Composio + browser become possible.
- After **F3-5**: end-users watch the agent work in a side panel. Safety + observability story is intact.
- After **F3-6**: safe to enable for non-power-users. Dry-run mode + cost caps + domain whitelist + audit log.
- After **F3-7**: the agent works on canvas/SVG-heavy apps (Figma-style UIs, some banking sites) where DOM-only grounding fails.
...
For F3-1 implementation, the primary reference is **browser-use** (DOM-extraction logic). For F3-3 (LLM-facing tools), the primary reference is **Stagehand** (API shape). For F3-7 (vision fallback), the primary reference is **Anthropic's computer-use docs** — not the listed repos.

---

## Phase 3.2 — explicitly deferred

Not part of Phase 3.1; do NOT scope-creep into F3-1..F3-7. Phase 3.2 is its own initiative:
...
Phase 3.2 lands once Phase 3.1 has 6+ weeks of production usage and a clear demand signal for the additional capabilities. Don't pre-build it.
...
1. **Anti-bot detection.** Cloudflare/PerimeterX/etc. fingerprint CDP-driven sessions and serve CAPTCHAs. Phase 3.1 doesn't solve this — we punt with a clear "site appears to block automation" error and ask the user to take over. Mitigation longer-term: humanized typing rhythms, mouse-movement noise, user-agent rotation. Not Phase 3.1's job.
2. **2FA / login redirects.** When a session expires mid-run, the agent must detect the login wall and ask the user to re-auth — never try to type credentials. F3-6's safety preamble enforces this.
...
4. **Cost explosion.** Vision grounding is 10–100× more expensive than DOM. F3-7 must gate vision behind explicit fallback triggers + a hard per-session cost cap.
5. **Privacy.** The browser agent sees the user's logged-in pages. Screenshots may capture PII. F3-6's audit log must redact / hash sensitive fields; the live-preview UI must not auto-record to disk.

### Open questions (defer to F3-1 implementation kickoff)

- Does the existing CEF child-webview lifecycle support a "hidden" mode for unattended workflow runs (cron at 8am, user not present)? If not, headless Chromium becomes a hard dependency earlier than Phase 3.2.
...
These get answered as part of F3-1's design work, not deferred indefinitely.
...
- **Not a screen-recording surface.** Live preview during the run, yes. Post-hoc recording / replay: not Phase 3.1.
...
If all seven points work end-to-end on a real brokerage / portal site, Phase 3.1 is shipped.

---

## Reading order for the implementing agent

1. This primer (F3-overview).
2. `F3-1.md` (CDP primitives) — start here.
3. `F3-2.md` (page perception) — depends on F3-1.
4. `F3-3.md` (LLM tools) — depends on F3-1 + F3-2.
5. `F3-4.md` (workflow node) — depends on F3-3.
6. `F3-5.md`, `F3-6.md`, `F3-7.md` in parallel after F3-4 — independent of each other.
