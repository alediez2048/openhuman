...
| OQ-4 | Phase 2 trigger types beyond webhook + composio + channel_message? | ✅ **Resolved: no more in Phase 2** | webhook + composio_event + channel_message cover the immediate Phase-2 needs. Future trigger ideas (cron-via-API, file-watch, system-event) require their own ADR + ticket — they do not slip into the Phase 2 ticket set on their own. Locked 2026-05-24. |
| OQ-5 | Run-history retention default? | ✅ **Resolved: 30 days** | Soft-delete sweep at the workflows-domain boot path per FR-1.3.4 (deferred from F-2, lands in F2-14). User-facing override comes later if asked; do not build a per-workflow retention setting in Phase 2. Locked 2026-05-24. |
...
| OQ-7 | Node inter-step data passing: literal, templating, or expressions? | ✅ **Resolved: templating** | Templating syntax: `{{node.<id>.output.<jsonpath>}}` (JSONPath against the upstream node's `NodeOutput.body`) and `{{trigger.<jsonpath>}}` for the run's trigger payload. Templating substitutes into string fields of `NodeConfig::*` (URLs, headers, body, prompts) at run time. Literal pass-through (current Phase-1 behaviour for single-node workflows) stays; a full expression engine (CEL / JQ) is deferred until a concrete user pull. Locked 2026-05-24. |
...
| OQ-21 | Per-node retry policy shape? | ✅ **Resolved: max_attempts + Exponential backoff** | Shape: `{ max_attempts: u32, backoff: Backoff::Exponential { initial_ms: u32, max_ms: u32 } }`. Default `max_attempts = 1` (single attempt) to preserve F-7 behaviour for nodes that don't opt in. Retry budget applies per-node, not per-run. `on_error: Halt` (F-7 default) wins over the retry budget when exhausted — the run still terminates. Locked 2026-05-24. Phase-2 README originally labelled this OQ-13; renumbered here to avoid collision with the existing resolved OQ-13. |
| OQ-22 | Webhook payload available to `agent_prompt` nodes? | ✅ **Resolved: yes, via `{{trigger.payload}}`** | The trigger's payload (webhook body / composio_event raw / channel_message raw) is exposed to every downstream node via the same templating surface as OQ-7. `agent_prompt` nodes get it via `{{trigger.payload}}` substitution in their prompt strings; `condition` nodes can match against it; `http_request` nodes can pipe it. Payload size capped at 256 KB to avoid blowing the prompt budget; oversize payloads truncate with a `[truncated, original X bytes]` marker. Locked 2026-05-24. Phase-2 README originally labelled this OQ-14; renumbered here to avoid collision with the existing resolved OQ-14. |
