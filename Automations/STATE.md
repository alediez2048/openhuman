...
- **🔴 F-16 (active) — workflow executor still runs as the orchestrator, ADR-016 allowlist is fiction.** Discovered live 2026-05-21 22:13 when manual Run now on a Composio Gmail→Slack workflow fired Gmail correctly but the Slack half died silently. Root cause: `executor::run_agent_prompt` calls `Agent::from_config(config)` (= orchestrator agent), ignores the `NodeAgentDefinition.allowed_tools` it just built. Orchestrator picks `delegate_to_integrations_agent` instead of `composio_execute`; step still marked Succeeded because the agent returned text. Spec at `Automations/Tickets/phase-1-foundation/F-16.md`. ~7 hours. **Until F-16 lands: Composio-routed workflows are NOT actually end-to-end functional, even though the run-history view says they ran.**
- **Channel send + Webview send** — `channel_send` and `webview_account_send` tools are stubs returning "Phase 2 (F2-5) deferral" errors. Workflows touching Channel or Webview connections in their `allowed_connections` won't actually send messages; they'll fail loud with a clear reason. **Composio-routed channels work IN PRINCIPLE** (Slack, Discord, Telegram, etc. via Composio's `composio_execute`) — but the executor doesn't actually route through `composio_execute` until F-16 (above). Land F2-5 to unify Channel/Webview send.
