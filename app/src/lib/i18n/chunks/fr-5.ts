...
const fr5: TranslationMap = {
  ...
  // T-2 (Phase 2.5 Trust UX) — per-run outcome card strings.
  'workflows.outcome.in_progress': 'Running…',
  'workflows.outcome.succeeded': 'Succeeded',
  'workflows.outcome.failed': 'Failed',
  'workflows.outcome.cancelled': 'Cancelled',
  'workflows.outcome.timed_out': 'Timed out',
  'workflows.outcome.what_happened': 'What this run did',
  ...
  'workflows.outcome.open_message': 'Open',
  'workflows.outcome.open_file': 'Open',
  'workflows.outcome.open_record': 'Open',
  'workflows.outcome.open_calendar': 'Open',
  ...
  'workflows.failure.agent_narrated': 'The agent described what it would do ({chars} chars of text) but never actually emitted a tool call.',
  'workflows.failure.agent_narrated_hint': 'Common cause: the model is getting stuck in chain-of-thought. Try pinning the workflow to `agentic-v1` (Settings → workflow_node).',
  'workflows.failure.composio_rejected': '`{tool}` was rejected by the connected provider: {detail}',
  'workflows.failure.composio_rejected_hint': 'The provider’s reason is verbatim above — usually a parameter rename or missing field. Edit the workflow prompt to use the corrected argument.',
  'workflows.failure.model_unavailable': 'Model `{model}` isn’t available in your account.',
  'workflows.failure.model_unavailable_hint': 'Edit the workflow → set model_tier to one of: {tiers}.',
  'workflows.failure.llm_auth_failed': 'Your {provider} API key was rejected.',
  'workflows.failure.llm_auth_failed_hint': 'Update the key in Settings → Providers, then retry the run.',
  'workflows.failure.connection_expired': 'Your {provider} connection expired or was revoked.',
  'workflows.failure.connection_expired_hint': 'Reconnect {provider} in Settings → Connections, then retry the run.',
  'workflows.failure.tool_slug_invalid': 'The agent tried to use an invalid tool name (`{slug}`).',
  'workflows.failure.tool_slug_invalid_hint': 'Usually a model hallucination — retry the run. If it recurs, file a bug.',
