...
> - **Phase 4 (Campaigns + Workflow UX)** — **18 tickets drafted at `Automations/Tickets/phase-4-campaigns/` on 2026-05-26**. This is the **next implementation work**. Locked architecture decisions in the README (campaigns as first-class via Shape β, pluggable `EntityStore` trait with Sheets + Attio adapters, chat-primary creation, hybrid form/chat editor, modal connection-launching, pinned-context global chat). Replaces Canvas as the Phase 4 slot.
...
> - **Phase ordering decision (2026-05-26 revision):** P2 ✅ → **P4 (Campaigns) [NEXT]** → P3 (Browser Agent) → P5 (Business Entities) → P6 (Proactive). Canvas is permanently demand-gated, may never ship.
> - **🟡 NEXT WORK: start Phase 4 Campaigns (F4-1).** Read `Automations/Tickets/phase-4-campaigns/README.md` + `F4-overview.md` first. Locked architecture from the 2026-05-26 grill — do NOT re-litigate mid-execution. Start with F4-1 (Campaign types + lifecycle).
