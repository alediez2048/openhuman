...
> - **Phase 1** (Workflows Foundation, F-1 → F-17 shipped) + **Phase 1.5** polish — landed on `main` (F-17 landed locally 2026-05-22). End-to-end chat-driven create / run / history works for Composio-routed workflows; F-16 closed the executor-side ADR-016 allowlist enforcement gap; F-17 wired the memory↔doer loop (pre-run recall + post-run structured store with ground-truth-first drift detection + `entity_tags` forward-compat for Phase 5). Tickets: `Automations/Tickets/phase-1-foundation/`.
...
> - **🟡 NEXT WORK: start Phase 2.** F-17 is landed locally (closure in `Automations/Tickets/phase-1-foundation/DEVLOG.md`). Read `Automations/Tickets/phase-2-execution/README.md` and resolve brainstorm OQs before starting F2-1.
