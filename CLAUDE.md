...
> - **Phase 1** (Workflows Foundation, F-1 → F-19 shipped) + **Phase 1.5** polish — landed on `main`. End-to-end chat-driven create / run / history works for Composio-routed workflows; F-16 closed the executor-side ADR-016 allowlist enforcement gap; F-17 wired the memory↔doer loop (pre-run recall + post-run structured store with ground-truth-first drift detection + `entity_tags` forward-compat for Phase 5); F-18 made MCP server registration user-isolation safe (stale-handle guard + orphan migration RPCs); F-19 hardened MCP UX (structured tool errors that kill the chat-agent confabulation pattern + endpoint auto-probe on add). Tickets: `Automations/Tickets/phase-1-foundation/`.
> - **Phase 2** (Execution Depth — multi-node chains, real channel/webview send, retries, scheduler) — 16 tickets drafted at `Automations/Tickets/phase-2-execution/`, not started. Phase 2 is the next implementation work.
...
> - **Phase 5** (Structured Business Entities + Outcome Observability) — PLACEHOLDER at `Automations/Tickets/phase-5-business-entities/PLACEHOLDER.md`. Earliest start: after Phase 3 + 3–6 months of production `entity_tags` data (the F-17 forward-compat hook).
> - **Phase 6** (Proactive Agent — agent-initiated workflow proposals) — PLACEHOLDER at `Automations/Tickets/phase-6-proactive-agent/PLACEHOLDER.md`. The long-term North Star confirmed in the 2026-05-22 grill. Earliest start: after Phase 5 has been live 2–4 months.
> - **Phase ordering decision (2026-05-22 grill, Option B):** F-17 → P2 → P3 → maybe-P4 → P5 → P6. Browser Agent and Canvas were swapped from the original numbering (P3↔P4) to reflect actual implementation priority. Canvas is gated on explicit user demand. P5/P6 are placeholders that capture the architectural North Star so P2/P3 decisions stay compatible.
> - **🟡 NEXT WORK: start Phase 2.** F-17 / F-18 / F-19 all landed locally (closures in `Automations/Tickets/phase-1-foundation/DEVLOG.md`). Read `Automations/Tickets/phase-2-execution/README.md` + resolve the 5 brainstorm OQs (leans documented), then start F2-1 (Phase 2 scaffold — make `NodeKind` + `Trigger` variants reachable end-to-end without per-kind bodies).
