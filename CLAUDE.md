...
> - **Phase 1** (Workflows Foundation, F-1 → F-21 + Phase 1.5 polish) — shipped to `main`.
> - **Phase 2** (Execution Depth, F2-1 → F2-17 + F2-7b + F2-9b + F2-16b) — **shipped to `main` on 2026-05-26**. Multi-node chains, webhook/composio/channel triggers, retry policy, active_hours, soft-delete + retention, 5 starter templates RU-5..RU-9, Appium live-test spec. See `Automations/Tickets/phase-2-execution/DEVLOG.md`.
> - **Phase 4 (Campaigns + Workflow UX)** — **backend + UI baseline shipped to fork on 2026-06-09** (F4-1..F4-13 + F4-17 + F4-18 closure docs). Hero stack live: campaign types + lifecycle, SQLite store, EntityStore trait with Sheets & Attio adapters, `for_each` executor node, throttle, approval queue, drafter prompt + `entity_schema_inspect`, `/campaigns` list/detail/inline-editor, 3 starter templates RU-10/11/12. **Deferred follow-ups (not load-bearing for the hero use case):** F4-14 per-node-kind editors, F4-15 connection modal launcher, F4-16 pinned chat context, F4-18 hero E2E (depends on F4-16). See `Automations/Tickets/phase-4-campaigns/DEVLOG.md`.
> - **Phase 3** (Browser Agent — CEF-native CDP-driven UI automation) — overview + 7 sub-tickets drafted at `Automations/Tickets/phase-3-browser-agent/`. Executes AFTER Phase 4 per the 2026-05-26 ordering. Additive to Composio, not a replacement.
> - **Canvas (originally Phase 4)** — **superseded** by Phase 4 Campaigns. Drafts preserved at `Automations/Tickets/phase-4-canvas/` for reference. Demand never materialised; Phase 4's hybrid detail editor (F4-12..F4-16) may make Canvas permanently unnecessary.
> - **Phase 5** (Structured Business Entities + Outcome Observability) — PLACEHOLDER. Scope reduces post-Phase-4 since the entity-adapter pattern delivers structured entities. Phase 5 becomes: native `entities.db` adapter + observability surface.
> - **Phase 6** (Proactive Agent) — PLACEHOLDER. Long-term North Star. Builds on Phase 5.
> - **Phase ordering decision (2026-05-26 revision):** P2 ✅ → P4 backend + UI baseline ✅ → **P4 UI polish OR P3 (Browser Agent) [NEXT]** → P5 (Business Entities) → P6 (Proactive). Canvas is permanently demand-gated, may never ship.
> - **🟡 NEXT WORK:** either close the remaining Phase 4 UI polish (F4-14 per-node editors, F4-15 connect modal, F4-16 pinned chat, F4-18 hero E2E) OR move to Phase 3 (Browser Agent) per the original ordering. The Phase 4 hero use case (chat → campaign → /campaigns → detail → inline edit → starter templates) works end-to-end today on the fork. Read `Automations/Tickets/phase-4-campaigns/DEVLOG.md` for the full ticket-by-ticket map + deferred-work list before picking up the polish work.
