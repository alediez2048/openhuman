...
> - **Phase 2** (Execution Depth — multi-node chains, real channel/webview send, retries, scheduler) — 16 tickets drafted at `Automations/Tickets/phase-2-execution/`, not started. Starts after F-17 lands.
...
> - **🟡 NEXT WORK: implement F-17** (`Automations/Tickets/phase-1-foundation/F-17.md`) — wires workflows into the Memory Tree (pre-run recall + ground-truth-first post-run store + `entity_tags` forward-compat). ~2–2.5 days. Phase 2 does not start until F-17 lands.
