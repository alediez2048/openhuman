...
- **Composio + MCP tool errors render as verbatim `⚠ ... tool error` blocks (F-19 + F-20)** — `composio_execute` and any MCP-touching tool emit a structured `⚠ Composio tool error\n…` / `⚠ MCP tool error\n…` block on failure with stable `kind:` labels (`invalid_slug_shape`, `toolkit_not_enabled`, `auth_failed`, `endpoint_not_found`, …) and a per-kind `suggestion:` field. The orchestrator prompt has a non-negotiable verbatim-render contract: never paraphrase, never invent HTTP status codes or OAuth scope names, never replace the suggestion. When adding new tool surfaces that can fail, mirror this pattern (`composio/tool_errors.rs` + `tools/impl/network/mcp.rs::McpToolErrorKind` are the references) — free-form anyhow strings let the LLM confabulate plausible-sounding excuses on top.
- **Composio action slugs are `^[A-Z][A-Z0-9]*(_[A-Z0-9]+)+$`** — uppercase + underscore-separated, two or more segments. `composio_execute` rejects anything else pre-dispatch with `kind: invalid_slug_shape`. Toolkit names (`linkedin`, `slack`, `gmail`) are NOT slugs; real action slugs look like `LINKEDIN_CREATE_LINKED_IN_POST`, `GMAIL_SEND_EMAIL`. Use `composio_list_tools(toolkit='<x>')` to discover real slugs.
...
## Workflows & Automations Initiative (Phase 1 + Phase 1.5)

- **Orchestrator `[tools].named` is an explicit whitelist, NOT a fallback** — `src/openhuman/agent/agents/orchestrator/agent.toml` lists every tool the orchestrator can call. A globally-registered tool that's missing from this list is invisible to the agent. The agent will hallucinate "the tool doesn't exist" or search Connections instead of calling it. Symptom in the Phase 1 work: agent asked "no workflow builder shows up in any of the connection categories" despite all 10 tools being live. Fix landed in commit `4c54e649` adding the 6 propose tools + 4 read tools to `[tools].named`. **Every new agent tool needs both global registration (`src/openhuman/tools/ops.rs`) AND an entry in the orchestrator allowlist.**
- **`build_node_agent_definition` names tools by string** — F-8's `src/openhuman/workflows/executor.rs::build_node_agent_definition` constructs the agent_prompt sub-agent's allowlist by calling `connection_tool_name(connection_ref)` which returns `"channel_send"` / `"webview_account_send"` / `"composio_execute"`. If the named tool isn't actually registered, the sub-agent fails at first tool call with "tool not found". Phase 1 ships Channel/Webview as stubs (`src/openhuman/tools/impl/workflows/channel_send_stub.rs`, `webview_account_send_stub.rs`) that return a clear Phase 2 deferral error instead of silent failure (commit `b0e3b73c`).
- **Drafter agent invocation pattern (cron-style)** — From a non-Turn context (e.g. `WorkflowProposer`), invoke an LLM with `Agent::from_config(&config)` → `agent.set_event_context(...)` → `agent.run_single(prompt)` returning `String`. `proposer.rs::AgentDrafter` / `AgentUpdateDrafter` follow this exactly. Don't try to thread a full chat-runtime Turn — the cron pattern is the right reference.
- **Forcing fenced JSON output from an LLM** — Instead of registering a synthetic `emit_proposal` tool (would require harness changes), append a structural instruction to the system prompt telling the model to emit a single ```json``` fenced block. Parse it with the `extract_fenced_json` helper. See `OUTPUT_FORMAT_INSTRUCTION` const in `proposer.rs`.
- **`<workflow-preview kind="..." data='{json}'>` chat inline tag** — The propose tools return `ToolResult::success_with_markdown(json, markdown_body)` with `supports_markdown=true` so the markdown body propagates to the agent's output. The orchestrator prompt teaches the agent to **echo the tag verbatim** without modification. `app/src/pages/conversations/utils/format.ts::parseBubbleSegments` was extended to match `WORKFLOW_PREVIEW_RE` alongside the existing `<openhuman-link>` regex. Segments are emitted in document order. `AgentMessageBubble.tsx` renders each preview via `renderWorkflowPreview(payload)` from `app/src/components/workflows/preview/index.tsx`.
- **`is_connected` wildcard matching** — Templates ship with `account_id: ""` (empty = "any account of this provider"). Strict equality made every template show "Missing connectors" even when the user had a connected account. Fix in `src/openhuman/workflows/health.rs::matches_ref` — empty `account_id` / `channel_id` / `tool_name` on the request side is a wildcard against the snapshot. Also added `src/openhuman/workflows/bus.rs::recompute_all_workflows(config)` boot-time sweep called from `src/core/jsonrpc.rs` so stale health rows refresh on startup (commit `f0a2288c`).
- **Telegram chips amber after matcher fix** — Templates require `{type: "channel", provider: "telegram"}` (Telegram Bot API). User had Webview Telegram (login-detection only). This is a real mechanism mismatch, not a UI bug. Honest answer: blocked until F2-5 (webview outbound send). Do not "fix" by relaxing the matcher.
- **Seed-origin workflows: "Move to starter workflows", not "Delete"** — `app/src/components/workflows/WorkflowCard.tsx` overflow menu shows different action labels based on `workflow.origin`. F-5's dedup will auto-restore the template anyway, so framing it as "Move" matches the user's mental model.
- **`Show starter workflows` toggle when hidden** — `WorkflowsList.tsx` catalog section returned `null` when hidden with no way to re-show. Added compact toggle next to the header; persists in localStorage like other UI prefs.
- **Don't unilaterally scope-cut tickets** — During F-15 I labeled hero E2E + chat-runtime extension + real drafter swaps as "Phase 1.5" deferral without asking. User pushback: "What do you mean 1.5 lands?? we've completed all of phase 1". Rule: if a ticket's deliverables list says X, ship X. If X looks too big, ask before deferring.
- **Workflows ↔ Memory Tree loop (F-17 LANDED locally 2026-05-22)** — OpenHuman's "memory and doer" thesis: workflows now read prior-run context BEFORE acting and write structured outcomes AFTER, automatically. Three executor hooks:
  1. **Pre-run recall.** `executor::run_agent_prompt` calls `workflow_memory::recall_prior_runs(workflow_id, 3)` before invoking the agent; the last-3 chunks render as `## Prior runs of this workflow` Markdown prepended to the user-message prompt (NOT the system prompt — that keeps the prefix cache stable). Empty result emits `## No prior runs — this is the first execution.` so the LLM doesn't think it's missing context.
  2. **Per-call trace.** F-16's failure-counter subscriber upgraded to `subscribe_tool_call_recorder` which still bumps the failure counter (F-16's gate is unchanged) AND records every `ToolExecutionCompleted` event into a `Vec<ToolCallObservation>`. `NodeOutput.tool_calls` carries the trace through to the post-run builder.
  3. **Post-run store.** `persist_run_memory(...)` writes a `WorkflowRunMemory` chunk after agent returns + before step-row terminal update. Ground-truth-first: `actual` is built from the trace, `narrative` is the agent's text, `compute_drift` runs a regex/substring heuristic to detect "agent claims sent but trace shows failure"; when drift fires, `narrative_drift` populates + the next-run recall block renders `⚠ Narrative drift: ... — DO NOT assume the narrative is true` INSTEAD of the narrative text, so the next agent doesn't inherit the confabulation. `entity_tags` are auto from `allowed_connections` (`entity:source:slack`) merged with the agent's optional `## Entities touched` section.
  
  **Key invariants:**
  - **Agents do NOT call `memory_store` for run summaries themselves — the runtime does it.** Telling the LLM to call it would double-write. The workflow_node prompt teaches this explicitly.
  - **Memory namespace is `workflow/{id}` (slash, not colon).** `UnifiedMemory::sanitize_namespace` strips `:` to `_` at write time and `Memory::get` doesn't re-sanitize, so colon-separator chunks would silently fail round-trip. Per-run keys (`run:{run_id}`) keep `:` — keys aren't sanitized.
  - **Recall reads via `Memory::sqlite_conn()` with a direct SQL query, NOT `Memory::list`.** `UnifiedMemory::list` maps `title → content` (a documented quirk on line 447 of memory_trait.rs) which would surface titles instead of the canonical Markdown chunk.
  - **Memory writes are best-effort.** A failed `Memory::store` logs a warn and does NOT roll back the run's terminal status — the run-history record is authoritative, the memory chunk is a sweetener.
  - **`redacted_args` + `inner_status` not yet captured** — the `ToolExecutionCompleted` event doesn't carry them today. The schema reserves the fields (defaulting to `Null`/`None`) so a future event extension fills them in without a memory-format change.
  Full closure in `Automations/Tickets/phase-1-foundation/DEVLOG.md` under "F-17 Closure (2026-05-22)".
- **Agent identity matters in the workflow executor — F-16 LANDED `3b572f71`** — pre-F-16, `executor::run_agent_prompt` called `Agent::from_config(config)` which defaults to the **orchestrator** agent, so workflow runs inherited orchestrator's "delegate to integrations_agent for external services" instinct → LLM picked `delegate_*` instead of the `composio_execute` tool the workflow's `NodeAgentDefinition.allowed_tools` granted. The `def` parameter was read for the log line ("allowed_tools=11") then discarded — ADR-016's allowlist was fiction at runtime. Same family of bug as Phase 1.5's drafter-identity gotcha. **As shipped:** new `workflow_node` agent definition (worker tier, empty base allowlist by design) + `Agent::from_config_for_agent_with_tool_override(config, agent_id, allowed_tools)` builder method that REPLACES (not unions) `ToolScope` + executor swapped to use both. Step status is also now honest: a `DomainEvent::ToolExecutionCompleted{success:false}` observed during the run (denied or executed-with-error) forces `RunStatus::Failed`, even if the agent itself returned text. `NodeOutput.tool_failure_count` carries the count. **The test-override path in `run_agent_prompt` must stay INSIDE the subscriber scope** — moving it outside (the way I had it briefly) makes the F-16 honest-status logic untestable. The placeholder pattern "build a `def`, then ignore it" can resurface anywhere — when adding a new executor pattern, make `def` load-bearing by design or remove it.
- **`recompute_all_workflows` boot sweep + per-event recompute** — `bus.rs` subscribes to `ConnectionsChanged` for live recompute on user actions; the boot sweep covers state that changed while the app was closed (e.g. user revoked a Composio scope in the browser).
- **Phase 1 ticket index** — `Automations/Tickets/phase-1-foundation/F-1.md` through `F-15.md`. Each ticket has the "What Is This Ticket / Why It Matters / Deliverables / Files / Architectural Decisions / Estimated Time" structure. Phase 1.5 polish commits are documented in `Automations/STATE.md` (not yet backfilled into ticket-specific READMEs).
- **Workflow initiative phases — implementation ordering decided 2026-05-22 (Option B + grill outcome)** — Order is **F-17 → P2 → P3 → maybe-P4 → P5 → P6**.
  - **F-17 (LANDED locally 2026-05-22)** at `Automations/Tickets/phase-1-foundation/F-17.md` — Memory↔doer loop. Closure entry in `DEVLOG.md`.
  - **Phase 2 (drafted, NEXT to implement)** at `Automations/Tickets/phase-2-execution/` — multi-node chains + real Channel/Webview send + retries + scheduler tightening. 16 tickets.
  - **Phase 3 (drafted)** at `Automations/Tickets/phase-3-browser-agent/` — CEF-native CDP-driven browser agent with Stagehand-style act/extract/observe API. Overview + 7 sub-tickets. Depends on Phase 2 only; NOT on Phase 4.
  - **Phase 4 (drafted, demand-gated)** at `Automations/Tickets/phase-4-canvas/` — visual canvas editor. May never ship.
  - **Phase 5 (PLACEHOLDER)** at `Automations/Tickets/phase-5-business-entities/PLACEHOLDER.md` — structured business entities + outcome observability. Earliest start: after Phase 3 + 3–6 months of F-17 `entity_tags` data.
  - **Phase 6 (PLACEHOLDER)** at `Automations/Tickets/phase-6-proactive-agent/PLACEHOLDER.md` — proactive agent (the long-term North Star: AI employee that notices patterns + proposes action with per-pattern trust gradient). Earliest start: after Phase 5 has been live 2–4 months.
  - Browser-agent + Canvas were originally numbered P4 ↔ P3; swapped 2026-05-22 to reflect actual implementation priority. Read STATE.md before picking up any phase.
