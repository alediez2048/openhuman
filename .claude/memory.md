...
- **Agent identity matters in the workflow executor (F-16)** — `executor::run_agent_prompt` calls `Agent::from_config(config)` which defaults to the **orchestrator** agent. That means workflow runs inherit orchestrator's "delegate to specialised sub-agents for external services" instinct → LLM picks `delegate_to_integrations_agent` instead of the `composio_execute` tool the workflow's `NodeAgentDefinition.allowed_tools` granted. The `def` parameter is read for the log line ("allowed_tools=11") then discarded — ADR-016's allowlist is fiction at runtime. Same family of bug as Phase 1.5's drafter-identity gotcha (drafter was also accidentally running as orchestrator). Fix lives in `Automations/Tickets/phase-1-foundation/F-16.md`: new `workflow_node` agent definition + per-instance `allowed_tools` override on the builder + honest step status from event-bus tap. Don't trust "step Succeeded" on a workflow run until F-16 lands — the agent may have returned text without any tool actually executing.
