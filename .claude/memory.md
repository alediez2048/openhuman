...
- **Workflows ↔ Memory Tree loop (F-17 drafted 2026-05-22)** — OpenHuman's "memory and doer" thesis requires workflows to read prior-run context BEFORE acting and write structured outcomes AFTER. F-16 added `memory_recall` + `memory_store` to the workflow_node baseline allowlist but didn't enforce use. F-17 makes the loop automatic at the executor level: `Memory::recall("workflow:{workflow_id}", limit=3)` prepended to the agent's user-message preamble before run; `Memory::store(WorkflowRunMemory)` called after run terminal. Per-workflow only in Phase 1.5; cross-workflow learning stays agent-opt-in via explicit `memory_store`. **Key invariant: agents do NOT call `memory_store` for run summaries themselves — the runtime does it.** Telling the LLM to call it would double-write. Spec at `Automations/Tickets/phase-1-foundation/F-17.md`. Should land before Phase 2 starts because multi-node chains compound the cost of NOT having memory (each node forgets prior nodes' history).
...
- **Workflow initiative phases — implementation ordering decided 2026-05-22 (Option B + grill outcome)** — Order is **F-17 → P2 → P3 → maybe-P4 → P5 → P6**.
  - **F-17 (drafted, NEXT to implement)** at `Automations/Tickets/phase-1-foundation/F-17.md` — Memory↔doer loop. ~2–2.5 days. Phase 2 does not start until F-17 lands.
  - **Phase 2 (drafted)** at `Automations/Tickets/phase-2-execution/` — multi-node chains + real Channel/Webview send + retries + scheduler tightening. 16 tickets.
